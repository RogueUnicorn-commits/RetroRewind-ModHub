...
internal static class SteamSecretStore
{
    private static string SecretPath = Path.Combine(AppContext.BaseDirectory, "Mods", "_downloads", "steam_api_key.dat");
    private static readonly string LegacySecretPath = Path.Combine(AppContext.BaseDirectory, "_downloads", "steam_api_key.dat");

    public static void Configure(string modsRoot)
    {
        if (string.IsNullOrWhiteSpace(modsRoot)) return;
        SecretPath = Path.Combine(modsRoot, "_downloads", "steam_api_key.dat");
    }
    ...
    private static byte[] Protect(string value)
    {
        ...
        try
        {
            ...
            if (!CryptProtectData(ref input, null, IntPtr.Zero, null, IntPtr.Zero, 0, ref output))
                throw new SecurityException("Windows could not protect the Steam Web API key.");
    ...
    private static string Unprotect(byte[] protectedBytes)
    {
        ...
        try
        {
            ...
            if (!CryptUnprotectData(ref input, IntPtr.Zero, IntPtr.Zero, IntPtr.Zero, IntPtr.Zero, 0, ref output))
                throw new SecurityException("Windows could not unprotect the Steam Web API key.");
